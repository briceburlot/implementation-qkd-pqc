...
# =========================================================================== #
# Client ETSI GS QKD 014 (inchangé fonctionnellement)                         #
# =========================================================================== #
class SAEClient:
...
def orchestrate_master(client, peer_sae_id, channel, *,
    ...
    # --- Point 2a : notifier le key_ID au pair (canal classique) ------------
    channel.put("key_id", {"key_ID": key_id, "master_SAE_ID": client.sae_id})

    # --- Point 2b : établissement PQC ML-KEM-768 ----------------------------
    #   le maître est l'initiateur : il attend la clé publique de l'esclave,
    #   encapsule, et renvoie le ciphertext.
    peer_pub = _b64d(channel.get("pqc_pub")["public_key"])
    ciphertext, pqc_secret = ch.pqc_initiator_encapsulate(peer_pub)
    channel.put("pqc_ct", {"ciphertext": _b64e(ciphertext)})
    ...
    if wg:
        _bring_up_tunnel(final_key, channel, role="master", **wg)
    return final_key
...
def orchestrate_slave(client, master_sae_id, channel, *, wg=None):
    ...
    channel.put("pqc_pub", {"public_key": _b64e(responder.public_key())})

    # --- Point 1 : récupérer le key_ID puis la clé QKD (slave -> dec_keys) --
    key_id = channel.get("key_id")["key_ID"]
    qkd_key_b64 = client.get_dec_key(master_sae_id, [key_id])[0]["key"]
    print(f"[1] clé QKD récupérée key_ID={key_id}")

    # --- Point 2 (suite) : décapsuler le ciphertext du maître ---------------
    ciphertext = _b64d(channel.get("pqc_ct")["ciphertext"])
    ...
    # --- Point 4 : tunnel WireGuard, PSK = clé finale -----------------------
    if wg:
        _bring_up_tunnel(final_key, channel, role="slave", **wg)
    return final_key
...
def _bring_up_tunnel(final_key, channel, *, role,
    ...
    channel.put(f"wg_pub_{role}", {"public_key": pub})
    other = "slave" if role == "master" else "master"
    peer_pub = channel.get(f"wg_pub_{other}")["public_key"]
...
def main():
    ...
    if hybrid:
        channel = ClassicChannel(os.environ.get("CHANNEL_DIR", "/shared/chan"))
        wg = None
